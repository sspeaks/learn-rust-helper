use ex20_async_handshake::{perform_async_handshake, AsyncHandshakeError, HandshakeReceipt};
...
async fn perform_handshake_success_decodes_receipt() {
    ...
    let receipt = async_call_or_hint!("ex20", "perform_async_handshake", {
...
async fn perform_handshake_invalid_status_maps_to_invalid_status_error() {
    ...
    let base_url = server.uri();
    let result = async_call_or_hint!("ex20", "perform_async_handshake", {
        let base_url = base_url.clone();
