...
fn poison_mutex(state: &Arc<Mutex<ex28_mutex_lockdown::LockdownState>>) {
...
fn shared_state_initializes_with_given_stability_and_zero_processed() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(25));
...
fn processing_empty_batch_keeps_state_unchanged() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(100));
...
fn single_event_updates_stability_and_processed_count() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(40));
    let outcome = call_or_hint!(
        "ex28",
        "process_lockdown_batch",
...
fn multiple_events_apply_all_deltas() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(10));
    let outcome = call_or_hint!(
        "ex28",
        "process_lockdown_batch",
        process_lockdown_batch(
...
fn zero_delta_events_still_increment_processed_counter() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(55));
    let outcome = call_or_hint!(
        "ex28",
        "process_lockdown_batch",
        process_lockdown_batch(
...
fn consecutive_batches_accumulate_on_shared_state() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(0));
...
fn negative_deltas_can_reduce_stability_below_zero() {
    let state = call_or_hint!("ex28", "shared_lockdown_state", shared_lockdown_state(3));
    let outcome = call_or_hint!(
        "ex28",
        "process_lockdown_batch",
