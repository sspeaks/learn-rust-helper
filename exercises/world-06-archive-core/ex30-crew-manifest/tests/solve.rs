...
fn load_empty_manifest_returns_empty_vec() {
    ...
    let loaded = call_or_hint!("ex30", "load_crew_manifest", load_crew_manifest(&conn))
...
fn load_without_schema_returns_sql_error() {
    ...
    let result = call_or_hint!("ex30", "load_crew_manifest", load_crew_manifest(&conn));
