use ex33_data_migration::{current_schema_version, migrate_schema, DataMigrationError};
...
fn migrate_to_zero_returns_no_steps() {
    ...
    let steps = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 0))
...
fn negative_target_version_is_rejected() {
    ...
    let result = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, -1));
...
fn migrate_to_one_sets_version_and_returns_one_step() {
    ...
    let steps = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 1))
...
fn migrate_to_same_version_is_idempotent() {
    ...
    call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 1))
        .expect("first migration should succeed");

    let again = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 1))
...
fn migrate_to_two_from_zero_returns_contiguous_steps() {
    ...
    let steps = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 2))
...
fn unsupported_high_target_does_not_change_current_version() {
    ...
    let result = call_or_hint!("ex33", "migrate_schema", migrate_schema(&conn, 999));
    assert!(
