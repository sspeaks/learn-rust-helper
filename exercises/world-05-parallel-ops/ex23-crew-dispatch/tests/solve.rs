use ex23_crew_dispatch::{run_crew_dispatch, CrewTask, CrewTaskResult};
...
fn empty_dispatch_returns_empty_results() {
    let results = call_or_hint!("ex23", "run_crew_dispatch", run_crew_dispatch(vec![]))
...
fn multiple_tasks_preserve_input_order() {
    ...
    let results = call_or_hint!("ex23", "run_crew_dispatch", run_crew_dispatch(tasks))
...
fn repeated_runs_are_deterministic_for_same_input() {
    ...
    let second = call_or_hint!("ex23", "run_crew_dispatch", run_crew_dispatch(tasks))
