use ex28_parallel_ops_capstone::{run_parallel_pipeline, PipelineJob};
...
fn output_count_matches_input_count() {
    ...
    let report = call_or_hint!("ex28", "run_parallel_pipeline", run_parallel_pipeline(jobs))
...
fn repeated_runs_with_same_jobs_are_deterministic() {
    ...
    let second = call_or_hint!("ex28", "run_parallel_pipeline", run_parallel_pipeline(jobs))
