# Quest 37: Archive Capstone

**🎮 Quest:** The mission is complete—almost. The final step is to fetch a batch of records from the remote archive network, then persist them to the local SQLite store. Two worlds collide: the async HTTP skills from World 5 and the synchronous database skills from World 7. But there's a catch: `rusqlite::Connection` cannot cross an `async` boundary. Design your solution to respect that constraint.
...
> **Schema precondition:** `persist_archive_batch` requires the archive table to have a `UNIQUE(mission_code, artifact)` constraint so that `INSERT OR IGNORE` can detect duplicates. Ensure this constraint is present when you create the table (see Quest 32).
...
Tests use a local **wiremock** mock server for HTTP and `Connection::open_in_memory()` for SQLite. No internet connection or system SQLite installation is required. The first build of World 7 exercises may take 30–45 seconds due to the bundled SQLite compilation. `fetch_archive_batch` tests use `#[tokio::test(flavor = "multi_thread")]`; `persist_archive_batch` and `load_archive_preview` tests are synchronous.
...
```bash
# Check your work
learn check ex37-archive-capstone

# Get a hint if stuck
learn hint ex37-archive-capstone

# Jump to a specific hint level
learn hint ex37-archive-capstone --level 2
```
...
- **Type Systems:** Methods, generics, and explicit lifetime annotations.
...
**Total campaign XP:** 11,570 | **Final rank:** Sovereign ⬢
